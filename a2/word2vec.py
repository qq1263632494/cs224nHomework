...
def negSamplingLossAndGradient(
    ...
    sgm1 = sigmoid(u_o @ centerWordVec)
    sgm2 = sigmoid( - u_k @ centerWordVec)
    loss = - np.log(sgm1) - np.sum(np.log(sgm2))
    gradCenterVec = (sgm1-1) * u_o + (1 - sgm2) @ u_k
    gradOutsideVecs = np.zeros(outsideVectors.shape)
    gradOutsideVecs[outsideWordIdx] = (sgm1-1)*centerWordVec
    for i, ind in enumerate(negSampleWordIndices):
        gradOutsideVecs[ind] += (1 - sgm2[i]) * centerWordVec
    ### END YOUR CODE
